...
### 회원 등록
- 회원 form을 만들고 Controller를 통해 매핑한다. 이름을 필수로 입력하게 하기 위해 validation을 사용한다. 이는 spring에서 제공하므로 이를 이용하면 된다.
- BindResult는 오류가 발생하여도 error를 띄우지 않고 그대로 실행 시킨다. 이때 결과를 처리해주면 된다.
- ![alt](membercontroller.JPG)


- ![alt](타임.JPG)

- 타임리프의 장점은 controller의 것을 그대로 가져와 html형삭으로 사용할 수 있다. 
- 엔티티를 최대한 dependacy없이 유지해야함. 엔티티는 비지니스 로직만 갖고 있어야함. 실무는 요구사항이 간단하지 않으니 form을 따로 만드는게 맞다. 즉 화면은 따로 있어야한다.API를 만들때는 멤버 엔티티를 반환해선 안된다.
...
- 변경 감지는 dirty checking 이라 함. 
##### 준영속 엔티티
- 영속성 컨텍스트가 더이상 관리하지 않는 엔티티
- 데이터베이스에 식별자가 있는 것
- -book 객체
- 이건 jpa가 관리하지 않아서 값을 바꿔도 db에 업데이트 되지 앦는다

#### 변경 감지
- ![alt](변경관리.JPG)
- jpa가 변경을 감지하고 update해줌

### 병합사용(merge) -> 위험
- 준영속성 엔티티를 영속으로
- 변경관리는 원하는 속성을 선택해서 변경할 수 있지만 병합을 사용하면 모든 속성이 자동으로 파라미터로 변경된다.
- ![alt](머지1.JPG)
- ![alt](병합1.JPG)
- 위 사진에서 price는 설정하지 않았기에null 값이 들어가 있는데 이가 그대로 update되어 들어간다.

- 따라서 실제로는 변경감지를 써야한다.
- 그런데 일일히 set으로 바꾸지 말고 change(,,) 처럼 메서드 하나를 만들어주고 이를 통해 바뀌야 나중에 쉽게 알 수 있다.

- 또 컨트롤러에서 어설프게 엔티티를 생성하지 말고, (위에 finditemr) 변경할 인자만 보내는게 좋다.
- ![alt](인자1.JPG)
- ![alt](인자2.JPG)